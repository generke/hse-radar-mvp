...
export async function POST() {
  try {
    ...
    const { data: member } = await supabase.from("memberships").select("organization_id, role").eq("user_id", user.id).limit(1).single();
    ...
    const session = await stripe.checkout.sessions.create({ mode:"subscription", customer_email:user.email, line_items:[{ price:process.env.STRIPE_PRICE_ID, quantity:1 }], success_url:`${base}/?payment=success`, cancel_url:`${base}/?payment=cancelled`, client_reference_id:member.organization_id, metadata:{ organization_id:member.organization_id }, subscription_data:{ metadata:{ organization_id:member.organization_id } } });
