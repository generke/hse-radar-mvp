...
export async function POST(request:NextRequest){
 try{
  ...
  const admin=createAdminClient(),hash=createHash("sha256").update(match[1]).digest("hex"),{data:pairing}=await admin.from("vision_telegram_pairings").select("id,organization_id,user_id,expires_at,used_at").eq("code_hash",hash).maybeSingle();
