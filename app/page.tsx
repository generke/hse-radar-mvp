...
import { Dashboard, type AdminAccessUser } from "@/components/dashboard";
...
export default async function Home({searchParams}:{searchParams:Promise<{org?:string}>}) {
  ...
  const [{data:platformAdmin},{data:membershipData}]=await Promise.all([
    ...
    supabase.from("memberships").select("*,organizations(id,name,plan,subscription_status,subscription_expires_at)").eq("user_id",user.id).order("created_at"),
  ]);
  ...
  const workspaces=isPlatformAdmin
    ...
    :memberships.filter(item=>item.is_active!==false).map(item=>{const org=related(item.organizations);return{id:item.organization_id,name:org?.name||"Организация",role:item.role,plan:org?.plan||"free",subscription_status:org?.subscription_status||"free",section_permissions:item.section_permissions||[]}});
  ...
  const [employees,inventory,ppe,documents,tasks,members,audit,paymentRequests,adminOrganizations,courses,questions,attempts,assignments,cameras,visionEvents,jobProfiles,notifications]=await Promise.all([
    ...
    supabase.from("memberships").select("*").eq("organization_id",selected.id).order("created_at"),
    ...
    supabase.from("learning_courses").select("*").eq("organization_id",selected.id).order("created_at",{ascending:false}),
  ...
  const team=memberRows.map(item=>({...item,full_name:names.get(item.user_id)||null,email:emails.get(item.user_id)||null}));
  let adminAccessUsers:AdminAccessUser[]=[];
  if(isPlatformAdmin){const{data:accessRows}=await supabase.from("memberships").select("organization_id,user_id,role,is_active,created_at").order("created_at",{ascending:false});const ids=[...new Set((accessRows||[]).map(item=>item.user_id))],{data:accessProfiles}=ids.length?await supabase.from("profiles").select("id,full_name,email").in("id",ids):{data:[]};const profileMap=new Map((accessProfiles||[]).map(item=>[item.id,item])),orgMap=new Map((allOrganizations||[]).map(item=>[item.id,item.name]));adminAccessUsers=(accessRows||[]).map(item=>{const profile=profileMap.get(item.user_id);return{...item,organization_name:orgMap.get(item.organization_id)||item.organization_id,full_name:profile?.full_name,email:profile?.email}}) as AdminAccessUser[]}
  ...
  return <Dashboard supabaseUrl={supabaseUrl} supabaseKey={supabaseKey} userEmail={user.email} userId={user.id}
    organizationId={selected.id} organizationName={selected.name} role={role} plan={selected.plan}
    isPlatformAdmin={isPlatformAdmin} workspaces={workspaces} sectionPermissions={isPlatformAdmin?[...sectionKeys]:(membership?.section_permissions||[])}
