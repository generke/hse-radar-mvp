...
type MembershipRow = {
  organization_id:string; role:string; created_at:string; section_permissions?:SectionKey[];
...
export default async function Home({searchParams}:{searchParams:Promise<{org?:string}>}) {
  ...
  const workspaces=isPlatformAdmin
    ...
    :memberships.map(item=>{const org=related(item.organizations);return{id:item.organization_id,name:org?.name||"Организация",role:item.role,plan:org?.plan||"free",subscription_status:org?.subscription_status||"free",section_permissions:item.section_permissions||[]}});
