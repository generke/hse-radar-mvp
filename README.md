...
Next.js-приложение для контроля работников, допусков, ТМЦ, обучения, задач, документов и событий Safety Vision.
...
Free ограничен пятью работниками, пятью единицами ТМЦ и пятью документами. Ограничение работает в интерфейсе и на уровне Supabase trigger.
...
Cron `/api/cron/deadline-notifications` запускается в 03:00 UTC — это 08:00 по времени Алматы. Он создаёт уведомления внутри платформы и, если настроены `RESEND_API_KEY` и `EMAIL_FROM`, отправляет тот же дневной список по электронной почте. Пользователь может отдельно разрешить системные уведомления браузера.
